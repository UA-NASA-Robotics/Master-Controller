...
int receiveArrayCAN_GLOBAL[10*GLOBAL_SYSTEM_DATA_SIZE];
int newDataFlag = 0;
...
void initCANFT(void) {
    ...
    beginCANFastGlobal(receiveArrayCAN_GLOBAL, sizeof (receiveArrayCAN_GLOBAL));
}
...
void setNewDataFlag(int index)
{
     newDataFlag |= (1 << index);
}
void clearNewDataFlag(int index)
{
    newDataFlag = newDataFlag & !(1 << index);
}
bool getNewDataFlagStatus(int mask)
{
    return ((newDataFlag & mask) == mask);
}
